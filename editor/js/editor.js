(function() {
  ...
  let currentLang = "en";
  let currentMainTab = "cards"; // "cards" | "menu" | "languages"
  ...
  function renderLangTabs() {
    ...
    langs.forEach(lang => {
      ...
      btn.addEventListener('click', () => {
        ...
        // languages tab has no per-lang content
      });
  ...
  function renderLanguageList() {
    ...
    data.settings.languages.forEach(lang => {
      ...
      div.querySelector('.remove-lang')?.addEventListener('click', (e) => {
        if (lang === 'en') return;
        if (confirm(`Remove language '${lang}'? This will delete all translations for that language from cards and menu.`)) {
          removeLanguage(lang);
        }
      });
  ...
  function removeLanguage(lang) {
    ...
    // Remove translations from cards
    data.cards.forEach(card => {
    ...
    input.addEventListener('change', (e) => {
      ...
      reader.onload = (ev) => {
        try {
          ...
          renderMenuList();
          refreshLandingSelect();
          document.getElementById('langTabs').querySelectorAll('.lang-tab').forEach(b => b.classList.remove('active'));
          document.getElementById('langTabs').querySelector(`[data-lang="${currentLang}"]`)?.classList.add('active');
