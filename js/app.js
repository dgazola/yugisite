...
async function init() {
  ...
  // Determine start card: URL snap param > landing card > home
  const urlParams = new URLSearchParams(window.location.search);
  const snapId = urlParams.get('snap');
  let startIndex = -1;
  if (snapId) {
    startIndex = state.allCardEls.findIndex(c => c.id === snapId);
  }
  if (startIndex < 0) {
    startIndex = state.allCardEls.findIndex(c => c.id === state.landingCardId);
  }
  if (startIndex < 0) {
